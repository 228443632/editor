...
import { COMP_PARAMS_NAME_MAP, COMP_SIGN_STYLE } from '@/examples/extensions/constant'

const { getTop } = useZIndexManage()

export const NAME = 'compSign' as const
...
declare module '@tiptap/core' {
  interface Commands<ReturnType> {
    insertCompSign: {
      insertCompSign: (options: TSetCompOptions) => ReturnType
    }

    insertCompSignByAttrs: {
      insertCompSignByAttrs: (attrs: TSetCompOptions['attrs']) => ReturnType
    }
  }
}
...
const defaultAttributes = {
  dragAttrs: {
    width: COMP_SIGN_STYLE.width,
    height: COMP_SIGN_STYLE.height,
    left: 0,
    top: '0',
    angle: null,
    rotatable: false,
